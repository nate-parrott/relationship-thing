...
def export_features_as_js(observations, fields, filename):
        dicts = [{field: ob.get(field) for field in fields} for ob in observations.itervalues()]
        f = open(filename, 'w')
        f.write("DATA = " + json.dumps(dicts))
...
def main():
        ...
        export_features_as_js(obs, all_fields, 'static/data.js')
